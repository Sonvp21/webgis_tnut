                                ...
                                <ul class="flex flex-col pl-0 mb-0 rounded-lg">
                                    @foreach ($dossierList as $dossierList)
                                        <li
                                            ...
                                            <a class="inline-block py-3 pl-0 pr-4 mb-0 ml-auto font-bold text-center uppercase align-middle transition-all bg-transparent border-0 rounded-lg shadow-none cursor-pointer leading-pro text-xs ease-soft-in hover:scale-102 hover:active:scale-102 active:opacity-85 text-fuchsia-500 hover:text-fuchsia-800 hover:shadow-none active:scale-100"
                                                href="{{ route('web.dossier.kythuat.edit', $dossierList) }}">Cập
                                                nhật</a>
                                        </li>
                                        <hr
                                            class="h-px bg-transparent bg-gradient-to-r from-transparent via-black/40 to-transparent">
                    ...
                    <div class="w-full max-w-full px-3 lg-max:mt-6 xl:w-4/12 mb-4 draggable" draggable="true">
                        <div>
                            <div class="grid grid-cols-1 gap-1" style="width: -webkit-fill-available;">
                                ...
                                <form action="{{ route('web.dossier.sangkien.update', $initiative) }}" method="POST">
                                    @csrf
                                    @method('PUT')
                                    <x-text-input label="Tên sáng kiến" name="name" placeholder=""
                                        value="{{ old('name', $initiative->name) }}" />

                                    <x-text-input label="Tác giả" name="author" placeholder=""
                                    ...
                                    <x-text-input label="Lĩnh vực" name="fields" placeholder=""
                                        value="{{ old('fields', $initiative->fields) }}" />

                                    <!-- Hồ sơ sáng kiến (Chỉ một hồ sơ) -->
                                    <div class="mb-4">
                                        <label class="font-medium text-lg">Thông Tin Hồ Sơ</label>
                                        <div>
                                            <x-text-input label="Tên hồ sơ" name="dossier[name]" placeholder=""
                                                value="{{ old('dossier.name', $initiativeDossier->name) }}" />

                                            <x-text-input label="Ghi chú hồ sơ" name="dossier[comment]" placeholder=""
                                                value="{{ old('dossier.comment', $initiativeDossier->comment) }}" />

                                            {{-- tệp đính kèm --}}
                                            <input type="hidden" id="documentData"
                                                value="{{ json_encode(
                                                    $initiativeDossier->documents->map(function ($doc) {
                                                        return [
                                                            'id' => $doc->id,
                                                            'name' => $doc->file_name,
                                            ...
                                            <label class="form-control ">
                                                <div class="label">
                                                    <span class="text-sm font-medium text-gray-700">Tệp đính kèm (tối
                                                        đa
                                                        10MB)</span>
                                                </div>
                                                <input type="file" class="filepond" multiple
                                                    data-allow-reorder="true" name="document_link[]"
                                                    id="document_link" />
                                                @error('document_link[]')
